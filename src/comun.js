...
//import {createPoints} from './api/create_p.js'
export default class Comun extends Phaser.Scene {
  ...
  preCarga(key, path) {
    ...
    this.scene.load.spritesheet("virtual_guy", "./assets/virtualguy.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.scene.load.spritesheet("fuego", "./assets/mapas/fire.png", {
      frameWidth: 16,
      frameHeight: 32,
    });
    this.scene.load.spritesheet("moneda", "./assets/mapas/coin.png", {
      frameWidth: 32,
      frameHeight: 32,
    });
    this.scene.load.spritesheet("flecha", "./assets/mapas/arrow.png", {
      frameWidth: 18,
      frameHeight: 18,
    });
    this.scene.load.spritesheet("portal", "./assets/mapas/portal.png", {
      frameWidth: 16,
      frameHeight: 16,
    });
    this.scene.load.spritesheet("heart", "./assets/mapas/heart.png", {
      frameWidth: 17,
      frameHeight: 17,
    });
  ...
  crear(
    ...
    console.log("es el final;", this.isFinal)
    /**----------------------------------------------------------------------------------------- */
    ...
    // Creación del mapa
    this.fondoJuego = this.scene.add.tileSprite(
      0,
      0,
      1056,
      704,
    ...
    this.spikeslayer = this.mapa.createDynamicLayer(
      "pinchos",
      this.spikestiles,
      0,
      0
    ...
    this.scoreText = this.scene.add.text(830, 33, "SCORE: " + this.puntos, {
      font: "30px Consolas",
      fill: "#FFF",
    });
    ...
    this.portal = this.mapa.createFromObjects("salida", "portal", {
      key: "portal",
    });
    ...
    this.monedas = this.mapa.createFromObjects("monedas", "moneda", {
      key: "moneda",
    });
  ...
  createArrowObjects() {
    ...
    this.flechas = this.mapa.createFromObjects("flechas", "flecha", {
      key: "flecha",
    });
  ...
  createSpringObjects() {
    ...
    this.trampolines = this.mapa.createFromObjects("trampolines", "trampolin", {
      key: "trampolin",
    });
  ...
  touchObject(player, tile) {
    ...
    if (this.death) {
      ...
      if (this.nCorazones === 0) {
        ...
        this.scene.scene.start(this.keyMundo, {
          vida: this.nCorazones,
          puntos: this.puntosInicio,
          contadorSegundos: this.contadorSegundos,
          email: this.email,
        });
      }
  ...
  exit(player, salida) {
    ...
    //aqui deberia haber algo que diga que el jugador ha llegado al final
    //let correo = this.email
    // y luego pasar estos datos al poinstcreate
    if(this.isFinal){
      console.log("entro aqui por primera vez")
      this.isFinal=false;
      console.log("is final exit;" , this.isFinal)
      let p = this.puntos;
      let k = 2;
      let c = this.nCorazones;
      let s = this.contadorSegundos;
      if(this.keyNexMundo == "Mundo4"){
        k = 3;
      }
      console.log("exit next mundo", this.keyNexMundo)
      if(this.keyNexMundo =="Final"){
        p = 10;
        k = 1;
        c = 5;
        s = 1;
      }
      console.log("exit p ", p)
      console.log("exit k ", k)
      console.log("exit c ", c)
      console.log("exit s ", s)
        pointsCreate(
          this.email,
          p,
          k,
          ...
          .then((result) => {
            console.log("entra aqui la m", result);
            this.scene.sound.stopAll();
            this.scene.sound.play("audio_portal");
            this.scene.scene.start(this.keyNexMundo, {
              vida: this.nCorazones,
              puntos: this.puntos,
              contadorSegundos: this.contadorSegundos,
              email: this.email,
  ...
  createHealt(n) {
    ...
    for (let i = 0; i < n; i++) {
      this.heart = this.scene.physics.add
        .sprite(620 + i * 40, 50, "heart")
        .setScale(1.5);
